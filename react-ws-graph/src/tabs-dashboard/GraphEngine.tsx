  ...
  const connectAll = () => {
    ...
    for (const uuid of uniqueUUIDs) {
      ...
      ws.onmessage = (ev) => {
        ...
        // Confirm stream index matches this UUID index
        const thisUuidIdx = uuidIndex.get(uuid);
        if (thisUuidIdx === undefined || thisUuidIdx !== idInt) {
          // If your backend always uses 0 for single-stream sockets, you can relax this check.
          // console.warn(`WS for ${uuid}: idInt ${idInt} != expected ${thisUuidIdx}`);
          return;
        }

        // Interpret fractional part. Backend sends 1-based? We support both.
        const candidates: number[] = [idFrac - 1, idFrac].filter((x) => x >= 0);

        // Push to ALL configs that include this waveform index by byteIndex.
        const cfgsForUuid = uuidToConfigs.get(uuid) ?? [];
        if (cfgsForUuid.length === 0) return;

        let delivered = 0;
        for (const cfg of cfgsForUuid) {
          // Find all lines whose byteIndex matches one of the candidates.
          const matches = cfg.lines.filter((ln) =>
            typeof ln.byteIndex === "number" && candidates.includes(ln.byteIndex as number)
          );
          if (matches.length === 0) continue;

          const patch: Record<string, number> = {};
          for (const ln of matches) {
            patch[ln.label] = value;
          }
          pushData(cfg.id, { ...patch, timestamp: t });
          delivered += matches.length;
        }

        if (delivered === 0) {
          // Helpful debug:
          // console.warn(`No matching line for uuid=${uuid} idFrac=${idFrac} (candidates=${candidates.join(",")})`);
        }
      };
